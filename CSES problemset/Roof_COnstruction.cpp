...
m=x-1;
//cout<<(1<<x)<<" "<<(1<<m)<<endl;
// ehat we do is sperate the last elements according to power of two
// as the 1st bit of the leargest consecutive elemets give a large no with the small
// eg  1001 * 0110 = 1111 
// so we have to close the 1st bit to zero so 1001 * 1000 = 0001 will be more efficient
// hence is the ans 
fr(i,n){
    if(v[i]<(1<<x) && v[i]>0) cout<<v[i]<<" ";
}
cout<<0<<" ";
fr(i,n){
    if(v[i]>=(1<<x)) cout<<v[i]<<" ";
