...
string s;
bigint dp[100001][9];
bigint recur(ll ind,ll val){
     if(ind==n){
          if(val==0) return bigint("1");
          else return bigint("0");
     }
     if(dp[ind][val]!=bigint("-1")) return dp[ind][val];
     bigint ans=bigint("0");
     if(s[ind]=='?'){
          for(int i=0;i<10;i++){
               ans+=recur(ind+1,(val+i)%9);
          }
     }else{
          ans=recur(ind+1,(val+(s[ind]-'0'))%9);
     }
