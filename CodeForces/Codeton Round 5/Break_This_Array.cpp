...
string s;
int dp[201][201][201];
ll recur(ll ind, ll L, ll R){
     if(ind==k){
          // cout<<L<<" "<<R<<" "<<ps[R+1]-ps[L]<<endl;
          return ps[R+1]-ps[L];
     }
     if(R<L) return 0;
     if(R==L) return v[R];
     ...
     ll ans=0;
     if(s[ind]=='L'){
          ans=mod_add(ans, recur(ind+1,L,R-1));
          ans=mod_add(ans,mod_mul(recur(ind,L,R-1),R-L-1));
     }else{
          ans=mod_add(ans,recur(ind+1,L+1,R));
          ans=mod_add(ans,mod_mul((ind,L+1,R),(R-L-1)));
     }
     ans= mod_div(ans,R-L);
     return dp[ind][L][R]=ans;
}
