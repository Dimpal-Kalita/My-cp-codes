...
void dk(){
     ...
     ll mul = 1;
     ll ans = 0 , N = b.size();
     ll cntt = 1, tmp = cnt[b[0]];
     if(m>N){
          cout<<0<<endl;
          return;
     }
     for(int i = 0 ; i < m ; i ++){
          mul = mod_mul(mul,cnt[b[i]],md);
     }
     pmul.pb(mul);

     for(int i = 1 ; i < N-m+1 ; i ++){
          mul = mod_mul(mul,cnt[b[i+m-1]],md);
          mul = mod_div(mul,cnt[b[i-1]],md);
          pmul.pb(mul);
     }
     
     for(int i = 0 ; i < N ; i ++){
          if(i+m-1 < N && b[i+m-1]-b[i] == m-1){
               ans = mod_add(ans ,pmul[i],md);
          }
     }
     cout<<ans<<endl;
