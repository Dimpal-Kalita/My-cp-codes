...
ll func(ll x , ll y , ll nx , ll ny , ll nx2 , ll ny2 , ll n){
	ll tmp1 = (x*x+mod)%mod;
	ll tmp2 = (y*y+mod)%mod;
	ll tmp3 = (tmp1+tmp2+mod)%mod;
	ll ans = (n*(tmp3)+mod)%mod;
	ll tmp4 = (nx*x+mod)%mod;
	ll tmp5 = (ny*y+mod)%mod;
	ll tmp6 = (tmp4+tmp5+mod)%mod;
	tmp6 = (2*tmp6+mod)%mod;
	ans = (ans-tmp6+mod)%mod;
	ll tmp7 = (nx2+ny2+mod)%mod;
	ans = (ans+tmp7+mod)%mod;
	return ans;
...
void solve(int tc =0)
{
	...
	for(int i = 0 ; i <  n;  i ++){
		...
		ll tmp1 =(v*v+mod)%mod;
		ll tmp2 = (u*u+mod)%mod;
		nx2 = (nx2+tmp2+mod)%mod;
		ny2 = (ny2+tmp1+mod)%mod;
	}
	...
	for(int i = 0 ;i < q ; i ++){
		ll u , v;
		cin >> u >> v;
		ans = (ans +  func(u , v ,nx , ny , nx2 , ny2 , n)+mod)%mod;
	}
	cout << ans << endl;
