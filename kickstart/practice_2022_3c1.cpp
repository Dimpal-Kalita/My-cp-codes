...
void solve(int tc =0)
{
	ll N , n , m , sy , sx; cin >> N >> n >> m >> sy >> sx;
    string s; cin >> s;
    map <pair<ll,ll> , data> mp;
    for(int i = 0 ; i < N ; i ++){
        if(s[i] == 'N'){
            if( mp[{sy,sx}].d == -1){
                 mp[{sy,sx}].d = min(sy+1 , n);
            }
            if(mp[{sy-1 , sx}].u == -1){
                sy--;
                mp[{sy , sx}].d = mp[{sy+1 , sx}].d;
            }else{
                ll tmp = sy;
                sy = mp[{sy , sx}].u;
                mp[{sy , sx}].d = mp[{tmp ,sx}].d;
            }
        }
        if(s[i] == 'S'){
             if(mp[{sy , sx}].u ==-1){
                mp[{sy , sx}].u = max(sy-1 , (ll)1);
             } 

            if(mp[{sy+1 , sx}].d == -1){
                 sy++;
                 mp[{sy , sx}].u = mp[{sy-1 , sx}].u;
            }else{
                ll tmp = sy;
                sy = mp[{sy , sx}].d;
                mp[{sy  , sx}].u = mp[{tmp , sx}].u;
            }
        }
        if(s[i] == 'E'){
           if(mp[{sy , sx}].l == -1){
            mp[{sy , sx}].l = max(sx-1 , (ll)1);
           }
            if(mp[{sy , sx+1}].r == -1){
                sx++;
                mp[{sy , sx}].l = mp[{sy , sx-1}].l;
            }else{
                ll tmp = sx;
                sx = mp[{sy , sx}].r;
                mp[{sy , sx}].l = mp[{sy , tmp}].l;
            }
        }
        if(s[i] == 'W'){
            if(mp[{sy , sx}].r == -1){
                 mp[{sy , sx}].r = min(sx+1 , m);
            }
            if(mp[{sy , sx-1}].l == -1){
                sx--;
                mp[{sy , sx}].r = mp[{sy , sx+1}].r;
            }else{
                ll tmp = sx;
                sx = mp[{sy , sx}].l;
                mp[{sy , sx}].r = mp[{sy , tmp}].r;
            }
        }
